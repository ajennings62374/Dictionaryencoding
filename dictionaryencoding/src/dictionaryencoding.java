...
public class dictionaryencoding {
    ...
    private static ArrayList<String> songwords = new ArrayList<>();
    ...
    public static void main(String[] args) {
        ...
        for (String word : input.split("[ \n]")) {
            if (!songwords.contains(word)) songwords.add(word);
        }

        for(String countMe: songwords) {
            int count = 0;
            for (String word : input.split("[ \n]")) {
                if (word.equals(countMe)) count++;
            }
            System.out.println("There are " + count + " instances of " + countMe);
        }
